...
void App::show()
{
	...
	std::cout << "Linha – Bloco – Endereço – Conteúdo (x representa espaço não utilizado)" << std::endl;
	...
	std::cout << "Bloco – Endereço – Conteúdo (conteúdo inicial preenchido aleatoriamente)" << std::endl;
...
Linha &App::subs_fifo(unsigned int indice_do_conjunto, unsigned int tamanho_do_conjunto)
{
	...
	if (linha_resultante == nullptr)
	{
		throw std::runtime_error("Não foi possivel manter a ordenação no conjunto (politica (FIFO/PEPS)");
	}
